...
find_package_complete() {
    ...
    printf "|\e[32m %-8s\e[0m |\e[32m %-14s\e[0m| %-32s| %-28s|\n" "✔" "${item_searched}" "${package}" "${version}"
}
...
exec_search() {
    ...
    for i in "$@"; do
        tool_installed=$(find /opt /usr -executable -type f -regextype posix-awk -iregex ".*/${i}")
        exit_code=$?
...
main() {
    ...
    output_main="$(exec_search "$@" 2> /dev/null)"
